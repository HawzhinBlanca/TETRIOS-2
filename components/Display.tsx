...
const Display: React.FC<Props> = ({ label, text, progress }) => {
  ...
  return (
    <div className="relative flex flex-col mb-6 group select-none">
      {/* Backplate */}
      <div className="absolute inset-0 bg-[#081020] skew-x-[-10deg] border-l-2 border-cyan-800/50 rounded-sm shadow-[inset_0_0_20px_rgba(0,0,0,0.8)]"></div>
      
      {/* Scanning Line Overlay */}
      <div className="absolute inset-0 skew-x-[-10deg] overflow-hidden opacity-20 pointer-events-none">
      ...
      <div className="relative z-10 p-4 flex flex-col w-full min-w-[200px]">
          ...
          <div className="relative">
              <span className="text-white text-3xl font-mono font-bold tracking-wider drop-shadow-[0_0_8px_rgba(6,182,212,0.4)] tabular-nums block text-right pr-2">
                  {text}
              </span>
              
              {/* Delta Indicator */}
              {delta && (
                  <span className="absolute top-0 left-0 text-sm font-mono font-bold text-green-400 animate-[fadeUp_0.8s_ease-out] drop-shadow-[0_0_5px_rgba(74,222,128,0.8)]">
                      {delta}
                  </span>
              )}
          </div>
          ...
          {progress !== undefined && (
              <div className="w-full h-1.5 bg-gray-900 mt-2 rounded-sm overflow-hidden border border-gray-800 relative">
                   {/* Ticks */}
      ...
      {/* Decorative Corners */}
      <div className="absolute top-0 left-0 w-2 h-2 border-t border-l border-cyan-600/50"></div>
      <div className="absolute bottom-0 right-0 w-2 h-2 border-b border-r border-cyan-600/50"></div>
    </div>
  );
};
